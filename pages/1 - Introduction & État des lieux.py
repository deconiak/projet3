...
image_icon = Image.open('logo_w.png')
...
st.write("""Les données de Datagouv et Datatourisme concernant le label “Qualité Tourisme” ne présentent pas le même nombre de points d’intérêt. 
L’objectif de notre étude est d’identifier les établissements non présents dans nos deux sources de données. 
""")
...
st.write("""Du côté Datatourisme, une grande partie des POI ont été mis à jour en 2022. 
    Tandis que les données de Datagouv la dernière mise à jour des données date de février 2021. 
    Il convient de garder en tête que la différence du nombre d'établissements Marque Qualité Tourisme  entre les deux données peut être en partie due à cette différence temporelle.""")
...
st.write("""Du côté Datatourisme, les catégories et sous-catégories de POI sont nombreuses (plus d’une centaine). 
    Ces catégories sont indiquées principalement en anglais avec la catégorie la plus présente étant les hébergements (accommodation, lodging business, hotel trade, camping etc.) 
    """)

st.write("""Du côté Datagouv, les catégories sont moins nombreuses (une dizaine environ). 
    La catégorie la plus représentée est celle des hôtels-restaurant. 
    """)
...
with col1 : 
    ...
    my_waffle(data_DT_MQ, "Marque Qualité\nDataTourisme")
...
with col2 : 
    ...
    my_waffle(data_DG_MQ, "Marque Qualité\nDataGouv")
...
st.caption("""nombre de poi = nombre POI Datatourisme - nombre de POI Datagouv. En vert: les régions et départements pour lesquels on retrouve plus de POI dans la base Datatourisme. 
En marron: les régions et départements pour lesquels on retrouve moins de POI chez Datatourisme par rapport à Datagouv. 
""")

st.write("""On met déjà en avant des régions ayant enregistré peu d’établissements “Marque Qualité Tourisme” sur la base Datatourisme: Auvergne-Rhône-Alpes et Provence-Alpes-Côte d’Azur. 
A l'inverse, le Grand-Est et les Pays de la Loire enregistrent plus d'établissements. 
""")
###########DATAFRAME#####################################
