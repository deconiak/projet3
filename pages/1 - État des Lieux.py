...
st.write("""Du côté Data.economie.gouv.fr, les catégories sont moins nombreuses (une dizaine environ) car elles sont calées sur le référentiel de la marque "Qualité Tourisme". La catégorie la plus représentée est celle des hôtels-restaurants. 
    """)
...
# tableau des régions 
poi_count_reg = pd.pivot_table(poi_count_total, index=["code_region", "nom_region"], values=["nb POI DATAtourisme", "nb POI Data.economie.gouv.fr", "difference"], aggfunc=np.sum, margins=False).reset_index()
